...
<?php
    ...
    if(isset($_POST['Usr-login']))
    {
      ...
      $u_pwd=($_POST['u_pwd']);//
      $stmt=$mysqli->prepare("SELECT u_email, u_pwd, u_id, u_status FROM tms_user WHERE u_email=? and u_pwd=? ");//sql to log in user
      $stmt->bind_param('ss',$u_email,$u_pwd);//bind fetched parameters
      $stmt->execute();//execute bind
      $stmt -> bind_result($u_email,$u_pwd,$u_id, $u_status);//bind result
      $rs=$stmt->fetch();
      ...
      if($u_status == "active"){
        ...
        {//get user logs
          ...
          {
            if($_GET['v_id']){
              header("location:user-confirm-booking.php?v_id=$_GET[v_id]");
            } else {
        ...
        // $error = "Access Denied. Your Account has been deactivated by the admin.";
    ...
    <div class="card card-login mx-auto mt-5">
      ...
      <div class="card-body">
        ...
        <div class="text-center">
          <a class="d-block small mt-3" href="register-terms.php">Register an Account</a>
