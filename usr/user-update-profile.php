  ...
  if(isset($_POST['update_profile']))
    {
           ...
           // $u_pwd=$_POST['u_pwd'];
            ...
            $query="update tms_user set u_fname=?, u_lname=?, u_phone=?, u_addr=?, u_category=? where u_id=?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('sssssi', $u_fname,  $u_lname, $u_phone, $u_addr, $u_category, $u_id);
            $stmt->execute();
                if($stmt)
                {
