  ...
  if(isset($_POST['upate_veh']))
    {
            ...
            $query="update tms_vehicle set v_name=?, v_reg_no=?, v_category=?, v_dpic=?, v_status=?, v_registration=? where v_id = ?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('ssssssi', $v_name, $v_reg_no, $v_category, $v_dpic, $v_status, $v_registration, $v_id);
            $stmt->execute();
                if($stmt)
                {
      ...
      <?php if(isset($succ)) {?>
        ...
        ?>
          <form method ="POST" enctype="multipart/form-data"> 
            ...
            <div class="card form-group" style="width: 30rem">
              <img src="../vendor/img/<?php echo $row->v_registration;?>" class="card-img-top" >
              <div class="card-body">
                  <h5 class="card-title">Registration Picture</h5>
