  ...
  if(isset($_POST['update_user']))
    {
            ...
            $has_sent_notif = "no";
            $query="update tms_user set u_fname=?, u_lname=?, u_phone=?, u_addr=?, u_category=?, u_email=?, u_pwd=?, has_sent_notif=? where u_id=?";
            $stmt = $mysqli->prepare($query);
            $rc=$stmt->bind_param('ssssssssi', $u_fname,  $u_lname, $u_phone, $u_addr, $u_category, $u_email, $u_pwd, $has_sent_notif, $u_id);
            $stmt->execute();
                if($stmt)
                {
                    $succ = "User Updated";
    ...
    <div id="content-wrapper">
      ...
      <?php if(isset($succ)) {?>
        ...
        <script>
                    setTimeout(function () 
                    { 
                        swal("Approve Payment","<?php echo $succ;?>!","success");
                    },
                        100);
        </script>
        ...
        <script>
                    setTimeout(function () 
                    { 
                        swal("Approve Payment","<?php echo $err;?>!","Failed");
                    },
                        100);
        </script>
        ...
        <div class="card-body">
          ...
          <?php
            ...
            $query = "SELECT * FROM `tms_transactions` WHERE `lessor_id` = ? ";
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="w-50 text-center">
                                            <a href="../uploads/payments/<?php echo $tr->trans_proof_of_payment ?>" class="btn btn-outline-info btn-sm" target="_blank"><i class="fa fa-eye"> </i> See Proof of Payment</a>
                                        </td>
                                        <td><?php echo $tr->trans_payment_status ?></td>
                                        <td>
                                            <?php if($tr->trans_payment_status == 'pending'): ?>
                                            <form method="POST">
                                                <input type="number" value="<?php echo $aid ?>" hidden />
                                                <button type="submit" class="btn btn-success btn-sm" name="approve_payment"><i class="fa fa-check-circle"></i> Approve Payment</button>
                                            </form>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
